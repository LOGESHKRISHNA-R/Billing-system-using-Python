...
class Bill_App:
    def __init__(self,root):
        ...
        lbl_color = 'white'
        #Title of App
        title = Label(self.root,text = "MNR STORE",bd = 12,relief = GROOVE,fg = fg_color,bg = bg_color,font=("times new roman",30,"bold"),pady = 3).pack(fill = X)
        ...
        #=================Customer Phone==============#
        cphon_lbl = Label(F1,text = "Mobile No",bg = bg_color,fg = fg_color,font = ("times new roman",15,"bold")).grid(row = 0,column = 2,padx = 20)
        ...
        #========chips
        wash_lbl = Label(F2,font = ("times new roman",15,"bold"),fg = lbl_color,bg = bg_color,text = "Chips")
        wash_lbl.grid(row = 2,column = 0,padx = 10,pady = 20)
        wash_en = Entry(F2,bd = 8,relief = GROOVE,textvariable = self.chips)
        wash_en.grid(row = 2,column = 1,ipady = 5,ipadx = 5)

        #========boost
        hair_lbl = Label(F2,font = ("times new roman",15,"bold"),fg = lbl_color,bg = bg_color,text = "Boost")
        hair_lbl.grid(row = 3,column = 0,padx = 10,pady = 20)
        hair_en = Entry(F2,bd = 8,relief = GROOVE,textvariable = self.boost)
        hair_en.grid(row = 3,column = 1,ipady = 5,ipadx = 5)

        #============Horlicks
        lot_lbl = Label(F2,font = ("times new roman",15,"bold"),fg = lbl_color,bg = bg_color,text = "Horlicks")
        lot_lbl.grid(row = 4,column = 0,padx = 10,pady = 20)
        lot_en = Entry(F2,bd = 8,relief = GROOVE,textvariable = self.horlicks)
        ...
        #===========Frame Content
        maza_lbl = Label(F2,font = ("times new roman",15,"bold"),fg = lbl_color,bg = bg_color,text = "Milo")
        maza_lbl.grid(row = 0,column = 0,padx = 10,pady = 20)
        maza_en = Entry(F2,bd = 8,relief = GROOVE,textvariable = self.milo)
        ...
        #========
        cold_lbl = Label(F2,font = ("times new roman",15,"bold"),fg = lbl_color,bg = bg_color,text = "Glucose")
        cold_lbl.grid(row = 3,column = 0,padx = 10,pady = 20)
        cold_en = Entry(F2,bd = 8,relief = GROOVE,textvariable = self.glucose)
        ...
        #========================
        genbill_btn = Button(F4,text = "Print Bill",bg = bg_color,fg = fg_color,font=("lucida",12,"bold"),bd = 7,relief = GROOVE,command = self.bill_area)
...
#Function For Text Area
    def welcome_soft(self):
        ...
        self.txt.insert(END,"       Welcome To MNR Store\n")
...
#Add Product name , qty and price to bill area
    def bill_area(self):
        ...
        if self.chips.get() != 0:
            self.txt.insert(END,f"\nHamburger         {self.chips.get()}           {self.chips.get() * 8}")
        if self.boost.get() != 0:
            self.txt.insert(END,f"\nHotdog            {self.boost.get()}           {self.boost.get() * 6}")
        if self.horlicks.get() != 0 :
            self.txt.insert(END,f"\nSandwich          {self.horlicks.get()}           {self.horlicks.get() * 4}")
        if self.wheat.get() != 0:
        ...
        if self.milo.get() != 0:
            self.txt.insert(END,f"\nGatorade          {self.milo.get()}           {self.milo.get() * 4}")
        if self.juice.get() != 0:
            self.txt.insert(END,f"\nJuice             {self.juice.get()}           {self.juice.get() * 2}")
        if self.coke.get() != 0:
            self.txt.insert(END,f"\nCoke              {self.coke.get()}           {self.coke.get() * 2}")
        if self.glucose.get() != 0:
            self.txt.insert(END,f"\nWaffer            {self.glucose.get()}           {self.glucose.get() * 2}")
